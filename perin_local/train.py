...
import numpy as np #changed - so loss can be shown
...
#changed - added loss save function
def save_loss(loss):
    with open("loss-log.txt","a") as f:
        f.write(str(loss) + "\n")
...
def main_worker(gpu, n_gpus_per_node, args):
    ...
    for epoch in range(args.epochs):
        ...
        for batch in dataset.train:
            ...
            losses_over_bs.append(total_loss.item()) #changed - added for analyzing loss
            total_loss.backward()
        ...
        print(str(lobs.mean()) + "; " + str(lobs.max()) + "; " + str(lobs.min())) #changed - print loss for epoch
        log.flush()
