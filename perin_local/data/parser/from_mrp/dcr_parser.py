...
class DcrParser(AbstractParser):
    def __init__(self, args, framework: str, language: str, part: str, fields, precomputed_dataset=None, filter_pred=None, **kwargs):
        ...
        utils.create_edge_permutations(self.data, DcrParser.node_similarity_key)
    ...
    def initialize(self, args, raw_path, cache_path, companion_path, precomputed_dataset=None):
        ...
        if precomputed_dataset is None:
            utils.create_possible_rules(data, DcrParser._create_possible_rules, prune=True)
            rule_set = utils.get_smallest_rule_set(data, approximate=self.language == "eng")
        else:
            utils.create_possible_rules(data, DcrParser._create_possible_rules, prune=False)
