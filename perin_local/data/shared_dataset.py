...
class SharedDataset:
    ...
    def share_vocabs(self, args):
        ...
        dcr_datasets = [dataset for (f, l), dataset in self.child_datasets.items() if f == "dcr"]
        if len(dcr_datasets) == 2:
            print("sharing DCR vocabs...")
            self.share_vocabs_(dcr_datasets[0], dcr_datasets[1], args, share_edges=True, share_anchors=True)
